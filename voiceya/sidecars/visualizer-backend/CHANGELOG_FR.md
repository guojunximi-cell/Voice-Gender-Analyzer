...
### Worker 边界行为（free mode + 非 fr 输入）

`engine_c_asr_fr` 用 faster-whisper multilingual ckpt + `language="fr"` pin。**对非法语 / 强噪音 / 极短录音，Whisper 可能解出空 transcript 或乱码**：

- 空 transcript → worker 直接 short-circuit：`Engine C skipped: empty transcript (noise / non-speech)` → `summary.engine_c=null`，Engine A 仍正常返回。
- 非空但语义乱码 → 进入 sidecar，MFA align 大概率低质量（`coverage` 低 / `phone_ratio` 极低）→ `low_quality=true`，前端 `metrics.alignWarning` 区出提示。

实测（N=1）：用一段中文音频跑 `language=fr-FR`，worker 日志见 `Engine C skipped: empty transcript`，最终响应 `summary.engine_c=null`，`summary.advice` 仍按 Engine A 数据生成 `tier=full`。**这不是 bug，是设计的优雅降级**。如果用户报"我录了法语但 Engine C 一直空"，先检查 ASR 转写是否非空（可在 worker log 中查 `engine_c_asr_fr` 上下文行）；常见原因是麦克风远 / 噪音大 / 录音 < 3 s。

Script mode 没有这条路径——绕开 ASR 直接用前端稿子，只要 sidecar 启动时 `stats_fr.json` 在场，就一定走 MFA。
