...
def trfbank(fs, nfft, lowfreq, maxfreq, nlinfilt, nlogfilt, midfreq=1000):
    ...
    heights = 2.0 / (frequences[2:] - frequences[0:-2])

    # Compute filterbank coeff (in fft domain, in bins)
    fbank = numpy.zeros((nfilt, int(numpy.floor(nfft / 2)) + 1), dtype=PARAM_TYPE)
    # FFT bins (in Hz)
    n_frequences = numpy.arange(nfft) / (1.0 * nfft) * fs
    ...
    for i in range(nfilt):
        ...
        lid = numpy.arange(
            numpy.floor(low * nfft / fs) + 1, numpy.floor(cen * nfft / fs) + 1, dtype=numpy.int32
        )
        left_slope = heights[i] / (cen - low)
        rid = numpy.arange(
            numpy.floor(cen * nfft / fs) + 1,
            min(numpy.floor(hi * nfft / fs) + 1, nfft),
            dtype=numpy.int32,
        )
...
def power_spectrum(input_sig, fs=8000, win_time=0.025, shift=0.01, prefac=0.97):
    ...
    framed = framing(input_sig, window_length, win_shift=window_length - overlap).copy()  # type: ignore
...
def framing(sig, win_size, win_shift=1, context=(0, 0), pad="zeros"):
    ...
    if pad == "zeros":
        return numpy.lib.stride_tricks.as_strided(
            numpy.pad(sig, c, "constant", constant_values=(0,)), shape=shape, strides=strides
        ).squeeze()
    elif pad == "edge":
        return numpy.lib.stride_tricks.as_strided(
            numpy.pad(sig, c, "edge"), shape=shape, strides=strides
        ).squeeze()
...
def pre_emphasis(input_sig, pre):
    ...
    if input_sig.ndim == 1:
        return (
            input_sig
            - numpy.c_[
                input_sig[numpy.newaxis, :][..., :1], input_sig[numpy.newaxis, :][..., :-1]
            ].squeeze()
            * pre
        )
    else:
...
def mfcc(
    ...
    spec, log_energy = power_spectrum(input_sig, fs, win_time=nwin, shift=shift, prefac=prefac)
    ...
    ceps = dct(mspec, type=2, norm="ortho", axis=-1)[:, 1 : nceps + 1]  # type: ignore
    lst = list()
    lst.append(ceps)
